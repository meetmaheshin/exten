...
function DeveloperDetailContent() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      apiFetch<UserDetail>(`/api/admin/activity/user/${userId}?from=${thirtyDaysAgo.toISOString()}`, { token: accessToken }),
      apiFetch<{ data: ScreenshotEntry[] }>(`/api/admin/screenshots?userId=${userId}&limit=12`, { token: accessToken }),
    ])
      .then(([detail, screenshotRes]) => { setData(detail); setScreenshots(screenshotRes.data); })
      .catch(console.error)
        ...
        <>
          ...
          <div className="card" style={{ marginBottom: 24 }}>
            ...
            <div className="table-container">
              <table>
                <thead><tr><th>Started</th><th>Duration</th><th>Active</th><th>Idle</th><th>Keystrokes</th><th>Saves</th></tr></thead>
                <tbody>
                  {data.sessions.map((s) => (
                    <tr key={s.id}>
              ...
              <div className="screenshots-grid">
                {screenshots.map((s) => (
                  <div key={s.id} className="screenshot-card">
                    <img src={`${API_BASE}/api/telemetry/screenshots/${s.id}/image?token=${accessToken}`} alt={s.filename} style={{ width: "100%", height: 140, objectFit: "cover", borderRadius: "8px 8px 0 0", display: "block" }} />
